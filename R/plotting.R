...
plot_fun <- function(plot_data, FDR, nhat) {
  ggplot2::ggplot(plot_data, ggplot2::aes_(x = ~sample_size, y = ~fdr_50_percentile)) +
    ggplot2::geom_point(colour = 4, size = 2) +
    ggplot2::geom_line(colour = 4, size = 1) +
    ggplot2::geom_ribbon(ggplot2::aes_(ymin = ~fdr_10_percentile, ymax = ~fdr_90_percentile),
                         linetype = 2,
                         alpha = 0.1,
                         colour = 4,
                         size = 1
    ...
    ggplot2::geom_vline(ggplot2::aes_(xintercept = nhat[1], linetype = paste(
      "Group 1: ", nhat[2],
      "\nGroup 2: ", nhat[3]
    )),
    colour = "darkblue", size = 1, key_glyph="blank"
    ) +
    ggplot2::labs(
      x = "Sample Size",
      y = "FDR",
    ...
    ggplot2::annotate(geom = "text", x = min(plot_data$sample_size), y = FDR, vjust = -1, label = paste("FDR = ", FDR)) +
    ggplot2::theme(
