...
metsize <- function(pilot = pilot, n1 = 4, n2 = 4, p = 200, prop = 0.25, covars = covars, ncovar = 0, model = "PPCA", plot.prop = FALSE, target.fdr = 0.05, Targeted = FALSE) {
  ...
  delta <- stats::qnorm(0.99) # Based on variance of underlying simulation model
  if (length(pilot) != 0) {
    ...
    for (k in 1:ntry)
    {
      ...
      if (m == 1) ## If influence of different proportion of significant metabolites is not of interest...
      {
        for (s in 1:Sim) ## assessing the effect of repeated simulations from the underlying model.
        {
          ...
          atsB <- abs(tsB)
          crit <- stats::quantile(atsB, pstat) ## identifying a cut-off point (m-th largest absolute value of the p TsB values)
          # (k) errors below colsums in numerator is for where atsB > crit and !ind means where is not truly significant
