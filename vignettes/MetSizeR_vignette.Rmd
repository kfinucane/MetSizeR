...
The MetSizeR package provides a R Shiny application which allows users to estimate the sample size required for their study to achieve a desired statistical power. The tool is built to estimate the sample size required for both targeted and untargeted metabolomic experiments. Sample size estimation is performed without the need for pilot data, however, if pilot data are available then this can be uploaded to the application to aid in the estimations. Several inputs are required by the user; it is expected that these inputs should be informed by the user's own knowledge of their field. Additionally, where pilot data are present it is expected that it has been sourced and treated appropriately in the context of the user's research.
...
Alternatively, MetSizeR can be downloaded and installed manually by the user if they so prefer by visiting [its entry on The Comprehensive R Archive Network (CRAN)](https://cran.r-project.org/web/packages/MetSizeR/).
...
![The `About` page for the MetSizeR application. A short introduction to the application is shown, along with references for methods used in its development. By clicking the checkbox shown, the user can choose to show more detailed information about the app's methods.](about_page.png){width=100%}
...
![Inputs for the sample size estimation method, shown for the case when pilot data are not available, and untargeted PPCA analysis is planned.](ssest_inputs.png){width=50%}
...
* Whether pilot data are available. The default is unavailable, indicated by the blank `Is experimental pilot data available?` checkbox.
...
![The covariate specification options to the sample size estimation algorithm, which appear when the `PPCCA` option is selected as the analysis model.](ppcca_options.png){width=50%}
...
![Instructions and options for uploading pilot data to the application, which appear once the `Is experimental pilot data available?` checkbox is ticked.](pilot_data_upload.png){width=50%}  
...
Below the plot, results will be displayed in text. The estimated optimal sample size will be printed, along with the per-group breakdown of the sample sizes, in the same ratio as input by the researcher. There is an option to download the plot, by clicking the `Download Plot` button. The plot will then download to the location of choice on the user's computer. There is also an option to view the exact values from the points on the plot by clicking the `Show values from plot?` checkbox. This opens a table showing the values of the 90th, 50th, and 10th percentiles of the FDR values calculated alongside the relevent sample size. The user can download this data as a CSV file by clicking the `Download Plot Data as .csv` button, which will download the
...
![The `Vary Proportion of Significant Spectral Bins` page for the MetSizeR application, which allows up to four proportions of significant variables to be evaluated for their optimal sample size. Inputs are specified using the options in the sidebar on the left of the screen, and results appear on the right of the screen once the computation is complete.](vprop_default.png){width=100%}
...
![The `Vary Proportion of Significant Spectral Bins` page with results shown for a set of inputs. Four proportions were tested and as such four plots are returned, each showing the result for a different significant proportion of bins.](vprop_results.png){width=100%}
